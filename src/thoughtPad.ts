...
export class ThoughtPad {
	...
	async recordThoughtOnDay(): Promise<void> {
	...
	async deleteEntry(entry: any): Promise<void> {
		this.thoughts.forEach((dt, dti) => {
			const index = dt.thoughts.findIndex(t => { return t.id === entry.id; });
			if (index >= 0) {
	...
	getThoughtsFile(): string {
		...
		if (!tFC || tFC === "") {
			vscode.window.showErrorMessage(
				"No Thoughts File configured in your Settings!"
			);
			return "";
		}
